...
export async function updateSession(request: NextRequest, requestHeaders?: Headers) {
  const nextResponseInit = requestHeaders ? { request: { headers: requestHeaders } } : { request };
  let supabaseResponse = NextResponse.next(nextResponseInit);
    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      cookies: {
        ...
        setAll(cookiesToSet) {
          ...
          supabaseResponse = NextResponse.next(nextResponseInit);
