...
import { Users, Building2, TrendingUp, Crown, Activity, ClipboardList, ToggleLeft } from "lucide-react";
...
export default async function SuperadminDashboard() {
  ...
  const [
    ...
    supabase.from("cron_run_logs").select("*", { count: "exact", head: true }).eq("status", "failed").gte("started_at", last24Hours),
    supabase.from("platform_audit_logs").select("*", { count: "exact", head: true }).gte("created_at", startOfToday),
  ]);
  ...
  const stats = [
    ...
    { label: "MRR", value: `GBP ${mrr.toLocaleString()}`, icon: <TrendingUp size={20} className="text-cr-sage" /> },
  ...
  return (
    <div>
      <CRPageHeader
        title="Superadmin"
        subtitle="Platform-wide metrics, health, audit trail, and rollout controls."
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <a href="/superadmin/health" className="cr-btn-secondary px-5 py-2.5 text-sm inline-flex items-center gap-2">
          <Activity size={15} /> Health {failedCronRuns ? `(${failedCronRuns})` : ""}
        </a>
        <a href="/superadmin/audit-log" className="cr-btn-secondary px-5 py-2.5 text-sm inline-flex items-center gap-2">
          <ClipboardList size={15} /> Audit log {auditEventsToday ? `(${auditEventsToday})` : ""}
        </a>
        <a href="/superadmin/feature-flags" className="cr-btn-secondary px-5 py-2.5 text-sm inline-flex items-center gap-2">
          <ToggleLeft size={15} /> Feature flags
        </a>
      </div>
    </div>
  );
