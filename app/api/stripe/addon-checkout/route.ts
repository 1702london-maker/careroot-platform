...
export async function POST(req: NextRequest) {
  ...
  const setupPriceId = WL_SETUP_PRICE_MAP[addon_type] ? process.env[WL_SETUP_PRICE_MAP[addon_type]] : null;
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: { organisation_id: orgId, addon_type, quantity: String(quantity) },
    success_url: `${appUrl}/dashboard/billing?addon_success=1`,
    cancel_url: `${appUrl}/dashboard/billing`,
    allow_promotion_codes: true,
    integration_identifier: "careroot-addon-checkout-efgh5678",
  });
